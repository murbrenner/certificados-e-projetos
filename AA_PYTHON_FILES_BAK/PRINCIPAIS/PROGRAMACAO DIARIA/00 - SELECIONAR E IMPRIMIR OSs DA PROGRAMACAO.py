...
from fiscais import fiscais, num_nomes, num_nomes_joined
...
with open(teste, mode="w", newline="") as teste:
    ...
    while j <= 35:                     
        try: 
            driver.find_element(By.XPATH, "href='javascript:facilitador(this,'DIMASDIASCOSTA')")#.format(fiscal[j]))
            print(num_nomes_joined[j])
            time.sleep(1111)
            j = j + 1
