...
for i in df.index:
    ...
    num_hd = driver.find_element(By.XPATH, "/html/body/form/table[3]/tbody/tr/td[2]/table[3]/tbody/tr[5]/td/table/tbody/tr[3]/td[3]").text
    if roteirizacao == 'IMÓVEL INEXISTENTE':
        print(i+1, str(df['MATRICULA'][i]), 'IMÓVEL INEXISTENTE', sep=',')
    else:
        print(i+1, str(df['MATRICULA'][i]), com_sem_hd, num_hd, sep=',')
