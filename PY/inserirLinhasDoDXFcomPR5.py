#NOME: Inserir LINHAS com pontos de referência no DXF (PR5)

#DESCRIÇÃO: Importa um arquivo DXF georreferenciado utilizando pontos de referência PR1 ate PR5. O script identifica os pontos PR1..PR5 no DXF, solicita ao usuário que clique nos pontos correspondentes no mapa, e aplica uma transformacao afim para aumentar a precisao.

#PRÉ-REQUISITO: Inserir camadas PR1, PR2, PR3, PR4 e PR5 no DXF marcando os pontos de referência.


# IMPORTADOR DXF COM GEOREFERENCIAMENTO POR PR1..PR5 (LINHAS)
# Seleciona DXF, identifica PR1..PR5 e gera camada de linhas da layer LINHAS
# Data: Mar/2026


def processar_dxf_overley_linhas():
    """
    Fluxo:
    1) Seleciona arquivo DXF
    2) Carrega camada DXF (entities)
    3) Encontra pontos de referencia no DXF (Layer = PR1, PR2, PR3, PR4 e PR5)
    4) Filtra Layer = LINHAS e prepara geometrias de linha
    5) Solicita 5 cliques no mapa (destino PR1..PR5)
    6) Aplica transformacao afim por minimos quadrados
    7) Salva shapefile de linhas ajustadas na pasta SHAPES e adiciona ao projeto
    """

    print("INICIANDO GEOREFERENCIADOR DXF (PR1..PR5) - LINHAS")
    print("=" * 70)
    ...
    try:
        ...
        def _filtrar_preselecao_por_comprimento(layer, limite_m):
            ...
            for feat in preselecionadas:
                ...
                if comp_m < limite_m:
                    ids_filtrados.append(feat.id())

            layer.selectByIds(ids_filtrados)
            return len(preselecionadas), len(ids_filtrados)

        def _remover_linhas_selecionadas_por_medidas(layer, medidas_alvo_m, tolerancia_m):
            if not layer or not layer.isValid():
                return 0
        ...
        def _remover_linhas_menores_que_wgs84(camada, comprimento_minimo_m, transform_context):
            if not camada or not camada.isValid():
                return 0

            medidor = QgsDistanceArea()
            medidor.setSourceCrs(camada.crs(), transform_context)
            medidor.setEllipsoid("WGS84")
            if hasattr(medidor, "setEllipsoidalMode"):
                medidor.setEllipsoidalMode(True)
            ...
            for feat in camada.getFeatures():
                ...
                comprimento = medidor.measureLength(geom)
                comprimento_m = medidor.convertLengthMeasurement(comprimento, QgsUnitTypes.DistanceMeters)
                if comprimento_m < comprimento_minimo_m:
        ...
        def _remover_linhas_tamanhos_especificos_conectadas_a_maiores(camada, medidas_alvo, tolerancia_medida, tolerancia_topologica):
            if not camada or not camada.isValid():
                return 0
            ...
            for fid_inicial in info.keys():
                ...
                comp_set = set(componente)
                alvo_comp = {
                    f
                    for f in componente
                    if any(abs(info[f]["len"] - medida) <= tolerancia_medida for medida in medidas_alvo)
                }
                if not alvo_comp:
                    continue

                limite_maior = max(medidas_alvo) + tolerancia_medida
                maiores_comp = {f for f in componente if info[f]["len"] > limite_maior}
                if not maiores_comp:
                    continue

                # Remove somente linhas com os tamanhos-alvo quando houver linhas maiores na mesma componente.
                remover.update(alvo_comp)
        ...
        def _remover_linhas_pendentes_nas_extremidades(camada, tolerancia_topologica, max_iter=8):
            if not camada or not camada.isValid():
                return 0
            ...
            for _ in range(max_iter):
                ...
                for feat in camada.getFeatures():
                    ...
                    info[feat.id()] = {"len": comp, "n1": n1, "n2": n2}
                    no_para_fids.setdefault(n1, set()).add(feat.id())
                    no_para_fids.setdefault(n2, set()).add(feat.id())
                ...
                comprimentos = sorted(v["len"] for v in info.values())
                mediana = comprimentos[len(comprimentos) // 2]
                # Foco em "linhaszinhas" de ponta; evita cortar trechos principais.
                limite_curta = max(0.20, min(2.20, mediana * 0.22))

                remover_iter = set()
                for fid, d in info.items():
                    deg1 = len(no_para_fids.get(d["n1"], set()))
                    deg2 = len(no_para_fids.get(d["n2"], set()))
                    ponta_solta = (deg1 == 1 and deg2 >= 2) or (deg2 == 1 and deg1 >= 2)
                    if ponta_solta and d["len"] <= limite_curta:
                        remover_iter.add(fid)

                if not remover_iter:
                    break

                for fid in remover_iter:
                    camada.deleteFeature(fid)
                total_removidas += len(remover_iter)
        ...
        def _criar_camada_linhas_overley(camada, campo_layer, crs_saida, nome_camada):
            ...
            for feat in camada.getFeatures():
                if _normalizar(feat.attribute(campo_layer)) != "LINHAS":
                    continue
            ...
            return camada_linhas, total_linhas, convertidas, ignoradas_tipo
        ...
        def _calcular_transformacao_afim(origens, destinos):
            if len(origens) < 3 or len(destinos) < 3 or len(origens) != len(destinos):
                raise ValueError("Transformacao afim requer ao menos 3 pares de pontos.")
            ...
            for origem, destino in zip(origens, destinos):
                ...
                matriz.append([x, y, 1.0, 0.0, 0.0, 0.0])
                matriz.append([0.0, 0.0, 0.0, x, y, 1.0])
                vetor.append(xp)
                vetor.append(yp)

            coeficientes, _, _, _ = np.linalg.lstsq(np.array(matriz), np.array(vetor), rcond=None)
            a, b, c, d, e, f = [float(v) for v in coeficientes]
            return QTransform(a, d, b, e, c, f)

        def _resumo_erro_transformacao(matriz, origens, destinos):
            erros = []
            for origem, destino in zip(origens, destinos):
                p_calc = matriz.map(origem)
                dx = p_calc.x() - destino.x()
                dy = p_calc.y() - destino.y()
                erros.append((dx * dx + dy * dy) ** 0.5)

            if not erros:
                return 0.0, 0.0

            rmse = (sum(e * e for e in erros) / len(erros)) ** 0.5
            return rmse, max(erros)
        ...
        print("\nPASSO 2: Carregar DXF e filtrar Layer = LINHAS")
        ...
        camada_linhas_overley, total_linhas, convertidas, ignoradas_tipo = _criar_camada_linhas_overley(
            dxf_layer,
            campo_layer,
            crs_projeto,
            f"{dxf_path.stem}_LINHAS",
        )

        if total_linhas == 0:
            print("Nenhuma feicao encontrada com Layer = 'LINHAS' no DXF")
            print("Sugestao: no seu DXF, crie/renomeie a camada para exatamente 'LINHAS' e execute novamente.")
            return False

        if convertidas == 0 or not camada_linhas_overley:
            print("Foram encontradas feicoes 'LINHAS', mas nenhuma geometria valida para linhas")
            return False

        print(f"Feicoes Layer 'LINHAS' encontradas: {total_linhas}")
        print(f"Geometrias LINHAS convertidas para linha: {convertidas}")
        if ignoradas_tipo:
            print(f"Geometrias ignoradas por tipo nao suportado: {ignoradas_tipo}")

        refs = ["PR1", "PR2", "PR3", "PR4", "PR5"]
        origens_por_ref = {}
        for ref in refs:
            origem, qtd = _obter_ponto_referencia(dxf_layer, ref, campo_layer)
            if not origem:
                print(f"Nao foi possivel localizar {ref} no DXF")
                print("Verifique se ha feicoes com Layer = 'PR1', 'PR2', 'PR3', 'PR4' e 'PR5'")
                return False
            origens_por_ref[ref] = origem
            print(f"{ref} no DXF: {qtd} feicao(oes) | ponto medio ({origem.x():.3f}, {origem.y():.3f})")
        ...
        destinos_por_ref = {}
        for ref in refs:
            destino = _capturar_ponto(
                canvas,
                f"Referencia {ref}",
                f"Clique no ponto do mapa que representa o {ref}.",
            )
            if not destino:
                print(f"Processo cancelado na captura do {ref}")
                return False
            destinos_por_ref[ref] = destino
            print(f"{ref} destino: ({destino.x():.3f}, {destino.y():.3f})")

        print("\nPASSO 4: Calcular transformacao afim (PR1..PR5)")
        origens = [origens_por_ref[ref] for ref in refs]
        destinos = [destinos_por_ref[ref] for ref in refs]

        try:
            matriz = _calcular_transformacao_afim(origens, destinos)
        except Exception as erro_afim:
            print(f"Falha ao calcular transformacao afim: {erro_afim}")
            return False

        rmse, erro_max = _resumo_erro_transformacao(matriz, origens, destinos)
        print(f"Erro RMSE nos pontos de controle: {rmse:.4f} m")
        print(f"Erro maximo nos pontos de controle: {erro_max:.4f} m")

        print("\nPASSO 5: Aplicar transformacao nas linhas LINHAS")
        if not camada_linhas_overley.startEditing():
        ...
        for feat in camada_linhas_overley.getFeatures():
            ...
            camada_linhas_overley.changeGeometry(feat.id(), geom)

        if not camada_linhas_overley.commitChanges():
            camada_linhas_overley.rollBack()
        ...
        if project.mapLayer(camada_linhas_overley.id()) is None:
            project.addMapLayer(camada_linhas_overley)

        print("\nPASSO 5.1: Pre-selecionar linhas inseridas na camada LINHAS")
        camada_linhas_overley.selectAll()

        print("\nPASSO 5.2: Filtrar pre-selecao para linhas menores que 15m")
        pre_total, pre_filtradas = _filtrar_preselecao_por_comprimento(camada_linhas_overley, 15.0)
        print(f"Pre-selecionadas: {pre_total} | Selecionadas < 15m: {pre_filtradas}")

        print("\nPASSO 5.3: Remover linhas selecionadas com medidas do triangulo")
        removidas_triangulo = _remover_linhas_selecionadas_por_medidas(
            camada_linhas_overley,
            medidas_alvo_m=(5.182, 1.704),
            tolerancia_m=0.08,
        )
        print(f"Linhas removidas por medida selecionada: {removidas_triangulo}")
        camada_linhas_overley.removeSelection()

        print("\nPASSO 6: Inverter sentido das linhas")
        total_invertidas = _inverter_sentido_de_todas_as_linhas(camada_linhas_overley)
        print(f"Linhas com sentido invertido: {total_invertidas}")

        print("\nPASSO 10: Salvar shapefile ajustado")
        nome_saida = f"{dxf_path.stem}_LINHAS_AJUSTADO_PR1_PR5"
        camada_linhas_overley.setName(nome_saida)
        ...
        print(f"Linhas exibidas (Layer LINHAS): {camada_final.featureCount()}")
        print(f"Shapefile: {shp_path}")
        print("A camada exibida foi filtrada para Layer = 'LINHAS'")
        print("Campo 'Text' foi renomeado para 'seq_id' quando existente")
        print("Rotulos: seq_id com texto branco e buffer azul")
        print("Transformacao afim aplicada com PR1..PR5")
        print("O sentido das linhas foi invertido ao final do processo")
        print(f"Filtro de pre-selecao (< 15m): {pre_filtradas} de {pre_total}")
        print(f"Linhas removidas por medidas do triangulo: {removidas_triangulo}")
...
print("INICIANDO GEOREFERENCIADOR DXF PR1..PR5 PARA LINHAS...")
resultado = processar_dxf_overley_linhas()
