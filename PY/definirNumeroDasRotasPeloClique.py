#NOME: Definir Numero das Rotas Pelo Clique

#DESCRIÇÃO: Define valores e atualiza atributos das feicoes conforme a regra estabelecida. Requer interacao do operador por clique no mapa. Camadas envolvidas: 'ROTAS DE LEITURA'.

#PRÉ-REQUISITO: Carregar a camada 'ROTAS DE LEITURA' no projeto QGIS; manter ativa a ferramenta/interacao por clique durante a execucao.
...
layer_name = 'ROTAS DE LEITURA'
layer = QgsProject.instance().mapLayersByName(layer_name)[0]

# Ferramenta de clique
class ClickTool(QgsMapTool):
    def __init__(self, canvas, layer, rota_inicial, setor, gerencia, janela):
        super().__init__(canvas)
        self.canvas = canvas
        self.layer = layer
        self.rota = rota_inicial
        self.setor = setor
    ...
    def canvasReleaseEvent(self, event):
        ...
        for feat in self.layer.getFeatures():
            if feat.geometry().contains(clicked_point):
                ...
                print(f"Feição atualizada: Rota {self.rota}")
                self.rota += 1
                break
...
class RotaDialog(QDialog):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Definir Rota, Setor e Gerência")
        ...
        self.form.addRow("Rota inicial:", self.rota_input)
    ...
    def ativar_ferramenta(self):
        ...
        self.tool = ClickTool(iface.mapCanvas(), layer, rota, setor, gerencia, self)
...
def iniciar():
    ...
    dlg = RotaDialog()
    iface._janela_rota = dlg  # <- MANTÉM A JANELA VIVA
    dlg.show()
