...
with open(relatorioGsan, mode="w", newline="") as relatorioGsan:
    ...
    for i in df.index:
        ...
        if matricula == 0:
            driver.get("http://g1.caema.ma.gov.br/gsan/exibirInserirImovelAction.do?menu=sim")
            ...
            try:
                ...
                try:
                    msg_erro = driver.find_element(By.XPATH, "/html/body/table/tbody/tr/td/table[3]/tbody/tr[1]/td[2]/span").text
                    row = i+1, msg_erro
                    writer.writerow(row)
                except:
                    ...
                    if categoria_res >= 1:                
                        driver.find_element(By.XPATH, "//select[@name='idCategoria']").send_keys('')
                        driver.find_element(By.XPATH, "//option[@value='1']").click()                
                        time.sleep(0.4)
                        driver.find_element(By.XPATH, "//select[@name='idSubCategoria']").send_keys('0') 
                        time.sleep(0.2)    
                        driver.find_element(By.XPATH, "//select[@name='idSubCategoria']").send_keys('0')                
                        time.sleep(0.4)
            ...
            try:
                ...
                row = i+1, "JA POSSUI MATRICULA [{}]".format(matricula)
                writer.writerow(row)
                pass
