#NOME: GERAR CSV A PARTIR DA ROTA COM LOGRADOURO
...
# Acessa as camadas
camada_imovel = QgsProject.instance().mapLayersByName('IMÓVEL')[0]
camada_ruas = QgsProject.instance().mapLayersByName('Ruas_MA')[0]
#amada_ruas = QgsProject.instance().mapLayersByName('ARRUAMENTO_MA')[0]

# Cria índice espacial das ruas
indice_ruas = QgsSpatialIndex(camada_ruas.getFeatures())

# Prepara ferramenta de medição
medidor = QgsDistanceArea()
medidor.setEllipsoid('WGS84')

# Mapeia as ruas por ID
ruas_dict = {f.id(): f for f in camada_ruas.getFeatures()}


cabeçalho = ['localidade', 'Setor', 'quadra', 'seq_id', 'rot_id', 'imv_id', 'latitude', 'longitude', 'alterar', 'logradouro']
    ...
    for rota in rotas_preselecionadas:
        ...
        for quadra in layerQuadras.selectedFeatures():
            ...
            for imovel in layerImovel.selectedFeatures():
                ...
                linha = localidade, (quadra['setor']), (quadra['quadra']), (imovel['seq_id']), (imovel['rot_id']), (imovel['imv_id']), (imovel['latitude']), (imovel['longitude']), '1', nome_rua
