#NOME: Inserir Overlay com pontos de referência no DXF
...
# IMPORTADOR DXF COM GEOREFERENCIAMENTO POR PR1/PR2 (LINHAS OVERLEY)
# Seleciona DXF, identifica PR1/PR2 e gera camada de linhas da layer OVERLEY
# Data: Mar/2026
...
def processar_dxf_overley_linhas():
    ...
    4) Filtra Layer = OVERLEY e prepara geometrias de linha
    ...
    print("INICIANDO GEOREFERENCIADOR DXF (PR1/PR2) - LINHAS OVERLEY")
    print("=" * 70)
    ...
    try:
        ...
        def _criar_camada_linhas_overley(camada, campo_layer, crs_saida, nome_camada):
            ...
            for feat in camada.getFeatures():
                if _normalizar(feat.attribute(campo_layer)) != "OVERLEY":
                    continue
            ...
            return camada_linhas, total_overley, convertidas, ignoradas_tipo
        ...
        print("\nPASSO 2: Carregar DXF e filtrar Layer = OVERLEY")
        ...
        camada_linhas_overley, total_overley, convertidas, ignoradas_tipo = _criar_camada_linhas_overley(
            dxf_layer,
            campo_layer,
            crs_projeto,
            f"{dxf_path.stem}_OVERLEY_LINHAS",
        )

        if total_overley == 0:
            print("Nenhuma feicao encontrada com Layer = 'OVERLEY' no DXF")
            return False

        if convertidas == 0 or not camada_linhas_overley:
            print("Foram encontradas feicoes 'OVERLEY', mas nenhuma geometria valida para linhas")
            return False

        print(f"Feicoes Layer 'OVERLEY' encontradas: {total_overley}")
        print(f"Geometrias OVERLEY convertidas para linha: {convertidas}")
        if ignoradas_tipo:
        ...
        print("\nPASSO 5: Aplicar transformacao nas linhas OVERLEY")
        ...
        print("\nPASSO 5.1: Pre-selecionar linhas inseridas na camada OVERLEY")
        ...
        nome_saida = f"{dxf_path.stem}_OVERLEY_LINHAS_AJUSTADO_PR1_PR2"
        ...
        print(f"Linhas exibidas (Layer OVERLEY): {camada_final.featureCount()}")
        print(f"Shapefile: {shp_path}")
        print("A camada exibida foi filtrada para Layer = 'OVERLEY'")
...
print("INICIANDO GEOREFERENCIADOR DXF PR1/PR2 PARA OVERLEY (LINHAS)...")
