#NOME: Inserir Pontos com Valores de Quadra DXF

#DESCRIÇÃO: Fluxo: 1) Seleciona arquivo DXF 2) Carrega camada DXF (entities) 3) Encontra pontos de referência no DXF (Layer = PR1 e PR2) 4) Solicita 2 cliques no mapa (destino PR1 e PR2) 5) Aplica transformação com translação + rotação + escala livre 6) Salva shapefile ajustado na pasta SHAPES e adiciona ao projeto. O resultado final e adicionado ao projeto.

#PRÉ-REQUISITO: Selecionar previamente as feicoes que serao processadas; garantir arquivo DXF georreferenciado compativel; manter ativa a ferramenta/interacao por clique durante a execucao.
...
def processar_dxf_completo():
    ...
    try:
        ...
        def _criar_camada_ponto_filtrada(camada, campo_layer, crs_saida, nome_camada):
            ...
            for field in camada.fields():
                ...
                if nome_origem.lower() == "text":
                    if "quadra" not in mapa_origem_destino:
                        campo_quadra = QgsField(
                            "quadra",
                            field.type(),
                            field.typeName(),
                            field.length(),
                        ...
                        campos_destino.append(campo_quadra)
                        mapa_origem_destino["quadra"] = nome_origem
                    continue
            ...
            for feat in camada.getFeatures():
                if _normalizar(feat.attribute(campo_layer)) != "TXT_QUADRA":
                    continue
        ...
        if camada_final.fields().indexFromName("quadra") != -1:
            settings = QgsPalLayerSettings()
            settings.enabled = True
            settings.fieldName = "quadra"
            ...
            print("✅ Rotulagem aplicada: campo 'quadra' | texto branco | buffer azul")
        else:
            print("⚠️ Campo 'quadra' não encontrado após exportação")
        ...
        print(f"📍 Pontos exibidos (Layer TXT_QUADRA): {total}")
        print(f"💾 Shapefile: {shp_path}")
        print("💡 A camada exibida foi filtrada para Layer = 'TXT_QUADRA'")
        print("💡 Campo 'Text' foi renomeado para 'quadra' na camada final")
        print("💡 Rótulos: quadra com texto branco e buffer azul")
